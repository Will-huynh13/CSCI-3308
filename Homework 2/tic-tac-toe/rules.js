...
function whoWins()
{
	...
	{
		return true; // this returns true if it is a diagonal from the top left to the bottom right
	}
	else if((board_state[2] == board_state[4]) && (board_state[4] == board_state[6]) && board_state[2] !== -1)
	{
		return true; // this returns true if the diagonal is from the top right to the bottom left
	}
	else
	{
        for(var i = 0; i < 3; i++) // this for loop is for a verticle win 
        {
            if((board_state[i] == board_state[i+3]) && (board_state[i+3] == board_state[i+6]) && board_state[i] !== -1)
            {
				return true;
			}
		}
		
        for(var i = 0; i < 9; i += 3) // this is for a horizontal win
        {
            if((board_state[i] == board_state[i+1]) && (board_state[i+1] == board_state[i+2]) && board_state[i] !== -1)
            {
				return true;
			}
		}
...
function begin_play()
{
	var pStart = document.getElementById("player1_id").value;
	var pStart2 = document.getElementById("player2_id").value;
	var player1Sign = " (X)";
	var player2Sign = " (O)";
	var str = "X";
	...
	else if(started != true)
	{
		...
		document.getElementById("player1_id").value += player1Sign;
		document.getElementById("player2_id").value += player2Sign;
...
{	
	...
	document.getElementById("move_text_id").innerHTML = "Cell Number for Example: A1";
...
function play()
{
	var table = ["A1", "A2", "A3", "B1", "B2", "B3", "C1", "C2", "C3"]; // array that checks input
	var firstMove =  table.includes(document.getElementById("move_text_id").value); // the includes here check if the entered value has it in the array
	var secondMove = board_state[table.indexOf(document.getElementById("move_text_id").value)] === -1; // board state
	var tableSpot = document.getElementById(document.getElementById("move_text_id").value); // made this to shorten the updating phase
	if (started == false) // if the game hasnt been started, print this
	{
		alert("The game has not started.");
	}
	else if (firstMove && secondMove) // this checks if the moves are valid 
	{
		board_state[table.indexOf(document.getElementById("move_text_id").value)] = whose_move(); // this is updaing ther boardstate
		if (whose_move() == 1)
		{
			tableSpot.innerHTML = "X";
		}
		else
		{
			tableSpot.innerHTML = "O";
		}
		countPlays++; // this counts the number of times the user moved

		if (whoWins()) // this checks who wins using a helper function defined as whoWins
		{
			alert("The winner of this game is " + CurPlayer); // this is printed if the function returns true at any of the conditions
			end_game = true;
		}

		toggle_move(); // this changes the player


		if (whose_move() == 1)
		{
			CurPlayer = "X"; // this sets the symbol so it can be used when someone wins
		}
		else
		{
			CurPlayer = "O";
		}
		document.getElementById("turn_info").innerHTML = "Turn starts for: " + CurPlayer.bold(); // this changes the text on the HTML page to the current player
	}
	else
	{
		alert("This is not a valid move, pick another spot.");
	}

	if (end_game == true || countPlays == 9) // if the table is full or the flag is set to true, the game resets
	{
		reset_play();
	}
}
